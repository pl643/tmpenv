...
function toggle_alt_status
    ...
    if test $ALTSTAT -eq 0    # currently off run on script
       tmux bind-key -n M-h select-pane -t :.- \; bind-key -n  M-j select-pane -D \; bind-key -n M-k select-pane -U \; bind-key -n  M-k send-keys Up \; bind-key -n  M-j send-keys Down \; \
            bind-key -n M-l select-pane -t :.+ \; set -g prefix C-a \; bind-key -n M-o next-window \; bind-key -n  M-i previous-window \; bind-key -n  M-u swap-pane -U \; bind-key -n M-z resize-pane -Z \; \
        ...
        tmux unbind -n M-h \; unbind -n M-j \; unbind -n M-k \; unbind -n M-l \; set -g prefix C-b \; unbind-key -n M-o \; unbind-key -n M-i \; unbind-key -n M-u \; unbind-key -n M-z \; \
